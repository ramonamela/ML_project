...
for(i in 1:length(filenames)){
  ...
  for(j in 1:length(C)){
    for(z in 1:length(alpha)){
      ...
      model.svm<- svm(as.factor(as.factor(Y.train[,pos])) ~ ., data=X.train.pca, type="C-classification", cost=j, gamma=z, kernel="radial", scale = FALSE)
